...
const index = ({ orderData, cityData, carts, provinceData, session, user }) => {
  ...
  const getCoupon = async (e) => {
    ...
    if (+order.totalPrice < +couponData.minSpend) {
      return setShowCouponMessage(
        `Minimal pembelanjaan adalah Rp. ${couponData.minSpend} untuk memakain diskon ini.`
      );
    }
  ...
  const selectCity = async (c) => {
    setCity(c);
    const province = await provinceData.find(
      (p) => p.province_id == c.province_id
    );
    ...
    const markup =
      data.value < 16000 || data.value == undefined ? 16000 : data.value;
  ...
  const selectPaymentMethod = (id) => {
    const data = paymentMethodCollection.find(
  ...
  const pay = async () => {
    ...
    if (choosenPaymentMethod == "COD") {
      ...
      const toShipping =
        area != "anotherCity"
          ? `${location} ${address}`
          : `${address}, ${postalCode}, ${city.type} ${city.city_name}, ${province.province}`;
    ...
    if (
      choosenPaymentMethod == "BNI" ||
      choosenPaymentMethod == "BRI" ||
      choosenPaymentMethod == "MANDIRI" ||
      choosenPaymentMethod == "PERMATA"
    ) {
      ...
      const selectedPaymentMethod = await invoiceResponse.available_banks.find(
        (bank) => choosenPaymentMethod == bank.bank_code
      );
    ...
    if (choosenPaymentMethod == "GOPAY") {
      const createGopayCharge = await axios.post(
        `/api/payment/midtrans/gopay`,
        {
          amount: total,
        }
      );
      ...
      return window.snap.pay(gopayResponse.resp.token, {
        onSuccess: async () => {
          const createTransaction = await axios.put(
            `${processs.env.NEXT_PUBLIC_API_URL}/transactions/${transactionResponse.id}`,
            {
              paymentStatus: "PAID",
            }
          );
    ...
    if (
      choosenPaymentMethod == "ID_DANA" ||
      choosenPaymentMethod == "ID_LINKAJA"
    ) {
      const createEWalletInvoice = await axios.post(`/api/payment/e-wallet`, {
    ...
    if (
      choosenPaymentMethod == "ALFAMART" ||
      choosenPaymentMethod == "INDOMARET"
    ) {
      const createRetailOutletInvoice = await axios.post(
        `/api/payment/retail-outlet`,
        {
          amount: total,
  ...
  return (
    <Layout title="Pembayaran">
              ...
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                  ...
                  <div className="w-full text-left md:text-left sm:mt-0 sm:ml-4 sm:text-left">
                    <h3
                      className="text-base md:text-lg leading-none font-medium "
                      id="modal-title"
                    >
                      Konfirmasi pesanan
                    </h3>
                    <div className="mt-2 flex flex-col space-y-2">
                      <p className="text-xs md:text-sm text-gray-500">
                        Apakah pesananmu sudah sesuai?
                      </p>
                      <br />
                      <div className="font-medium tracking-wide text-xs md:text-sm text-slate-500">
                        Produk
                      </div>
                      {orderData[0].products.map((product, index) => {
                        return (
                          <div
                            className="flex justify-between space-x-2 text-xs md:text-base"
                            key={index}
                          >
                            <div className="flex space-x-1">
                              <span className="w-2/12 md:w-10">
                                x {product.quantity}
                              </span>
                              <span className="w-10/12 line-clamp-1 flex-initial max-w-sm">
                    ...
                    <div className="flex text-xs md:text-base flex-col space-y-2">
                      <div className="flex justify-between space-x-2 md:space-x-4 font-medium">
                        <span className="truncate font-bold w-1/2">
                          Sub Total
                        </span>
                        <NumberFormat
                      ...
                      <div className="flex justify-between space-x-2 md:space-x-4 font-medium">
                        ...
                        <NumberFormat
                          className={`font-bold text-xs md:text-base ${
                            subTotal >= 200000 && area == "malang-batu"
                    ...
                    <div className="flex text-sm md:text-base flex-col space-y-2">
                      <div>
                        <div className="font-medium tracking-wide text-sm text-slate-500">
                          Lokasi
                        </div>
                        {area != "anotherCity" ? (
                          <>
                      ...
                      <div>
                        ...
                        <div className="font-medium">{`${choosenPaymentMethod
                          .replace("ID_", "")
                          .toUpperCase()} ${
                          ovoNumber && `(${ovoNumber})`
                        }`}</div>
      ...
      <div className="container mx-auto px-4 md:px-12 lg:px-16 my-4 md:my-5 2xl:my-6">
        ...
        <h1 className="text-lg md:text-2xl font-semibold my-2 2xl:my-3">
          Pembayaran
        </h1>
                      ...
                      <div className="flex items-center space-x-2 md:space-x-4 text-sm md:text-base">
                        {areaCollection.map((n, index) => {
                          return (
                            ...
                            >
                              {area != n.value && (
                                <div className="border border-slate-800 rounded-full w-4 h-4"></div>
                              )}
                              {area == n.value && (
...
export const getServerSideProps = async (contex) => {
  ...
  const getCart = await axios.get(
    `${process.env.NEXT_PUBLIC_API_URL}/carts?user=${session.id}`,
    {
      headers: {
        Authorization: `Bearer ${session.jwt}`,
      },
  ...
  const getOrder = await axios.get(
    `${process.env.NEXT_PUBLIC_API_URL}/orders?user_eq=${session.id}`,
    {
      headers: {
        Authorization: "Bearer " + session.jwt,
      },
  ...
  const getCity = await axios.get(
    `${process.env.NEXT_URL}/api/expedition/cities/`
  );
  const city = getCity.data;

  const getProvince = await axios.get(
    `${process.env.NEXT_URL}/api/expedition/provinces/`
  );
